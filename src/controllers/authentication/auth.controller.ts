...
const register = async (req: Request, res: Response) => {
    try {
        logger.info('Register process started.', { label: NAMESPACE });
        ...
        const errorMessage = getErrorMessage(err);
        logger.error(`Error occurred while registering user - ${errorMessage}`, { label: NAMESPACE });
...
const resetPassword = async (req: Request, res: Response) => {
    try {
        logger.info('Reset password process started.', { label: NAMESPACE });
        ...
        const errorMessage = getErrorMessage(err);
        logger.error(`Error occurred while resetting password to user - ${errorMessage}`, { label: NAMESPACE });
...
const newPassword = async (req: Request, res: Response) => {
    try {
        ...
        const errorMessage = getErrorMessage(err);
        logger.error(`Error occurred while recovering new password to user - ${errorMessage}`, { label: NAMESPACE });
...
const forgotPassword = async (req: Request, res: Response) => {
    try {
        logger.info('Password change process started.', { label: NAMESPACE });

        const email: string = req.body.email;
        const result = await userServices.forgotPassword(email);
           
        logger.info('Password changed successfully.', { label: NAMESPACE });
        res.status(200).json(result).end();
    } catch (err) {
        const errorMessage = getErrorMessage(err);
        logger.error(`Error occurred while resetting password to user - ${errorMessage}`, { label: NAMESPACE });
...
const confirmRegister = async (req: Request, res: Response) => {
    try {
        logger.info('Confirm registry process started.', { label: NAMESPACE });

        const confirmToken = req.headers['confirm-token'] as string;

        const result = await userServices.confirmRegistry(confirmToken);
        
        logger.info('Confirm registry successfully.', { label: NAMESPACE });
        res.status(200).json(result).end(); 
    } catch (err) {
        const errorMessage = getErrorMessage(err);
        logger.error(`Error occurred at confirm registry - ${errorMessage}`, { label: NAMESPACE });
